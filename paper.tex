...
\end{eqnarray}
where we have defined the following quantities, $Z_1=1+(1-a_*^2)^{1/3}[(1+a_*)^{1/3}+(1-a_*)^{1/3}]$, $Z_2=(3a_*^2+Z_1^2)^{1/2}$, and $a_*=a/M$, with $a$ and $M$ being the spin Kerr parameter and the black hole mass. \tf{Check that the above expressions are correct.}
...
\end{equation}
with $\eta_{\mathrm{max}} = l_{\mathrm{K}}(r_{\mathrm{mb}})/l_{\mathrm{K}}(r_{\mathrm{ms}})$. In this paper, and as it is done in~\citet{Qian:2009}, we choose $\eta = \eta_{\mathrm{max}}$, and then we can write $l_0$ as $l_0 = l_{\mathrm{K}}(r_{\mathrm{mb}})$. We take this choice because [explicar que esta eleccion hace que el momento angular esté comprendido entre lmb y lms para variaciones de beta entre 0 y 1. Además los discos de momento angular constante que empiezan en el cusp, no tienen borde externo para l = lmb] \tf{Write the explanation in the draft.}
...
\end{equation}
where $K_{\mathrm{m}}$ and $\eta$ are constants. \tf{I think we should use a different letter for the exponent instead of $\eta$ as it can be confused with the $\eta$ in the angular momentum ansatz.}
...
As in \citet{Komissarov:2006}, in this paper we have chosen the exponents $\kappa = \eta = 4/3$ and the enthalpy at the disk center $w_{\mathrm{c}} = 0$. That leaves us the magnetization parameter $\beta_{\mathrm{c}}$, the parameters of the angular momentum distribution $\beta$ and $\gamma$, the black hole spin parameter $a$ and the inner radius of the disk $r_{\mathrm{in}}$. This allows us to control the size, shape, thickness and magnetization of the disk.

\begin{table}
\caption{List of models. Form left to right the columns report: the black hole spin $a$, the parameters of the angular momentum distribution $\beta$ and $\gamma$, the magnetization parameter $\beta_{\mathrm{c}}$ for which a maximum pressure is achieved, the corresponding radii of the maximum thermal pressure and magnetic pressure, $r_{{\rm{max}}}$ and $r_{{\mathrm{m, max}}}$, the gravitational potential diference between the center and the inner radius, $\Delta W\equiv W_{\rm c}-W_{\rm in}$, and the inner and outer radii of the disks, $r_{\mathrm{in}}$ and  $r_{\mathrm{out}}$. \tf{Complete table.}}             
\label{table:1}      
\centering          
\begin{tabular}{c c c c c c  c c c c}
\hline\hline       
 & $a$ & $\beta$ & $\gamma$ & $\beta_{\mathrm{c}}$  & $r_{\rm{max}}$ &  $r_{\mathrm{m, max}}$ & $\Delta W$ & $r_{\mathrm{in}}$ & $r_{\mathrm{out}}$ \\ 
\hline           
A1 & $0.5$ & $0.0$ & $0.0$ &  &  &    &  &  & \\ 
A2 &           & $0.5$ & $0.5$ &  &  &    &  &  & \\ 
A3  &          & $0.9$ & $0.9$ &  &  &    &  &  &  \\ 
...
\caption{Radial profiles at the equatorial plane for models with $a=0.9$, $\beta_{\rm c}=10^{-3}$, and same combination of the $\gamma$ and $\beta$ parameters as in the rows of Fig.~\ref{models} (the specific values are indicated in the legend). \tf{Include a legend indicating the particular values of these two parameters.}}
\label{more-profile}
\end{figure}
...
\caption{Caption. \tf{Complete the caption.}}
           \label{max-vs-magnetization}%
 \end{figure*}
 
A representative sample of the isodensity surfaces (in logarithm scale) for some of our models appears in Figure~\ref{models}. From left to right the columns of this figure correspond to increasing values of the black hole spin, namely, $a=0.5$, 0.9 and 0.99, while from top to bottom the rows correspond to different combinations of the $\gamma$ and $\beta$ parameters that characterize the ansatz for the angular momentum distribution of~\citet{Qian:2009} (the particular values are indicated in the caption of the figure). The value of the magnetization parameter for all models in this figure is $\beta_{\rm c}=10^{-3}$. \tf{Is that correct?} A rapid inspection shows that the structure of the disks noticeable changes when the parameters change. Notice that the spatial scale in all of the plots of this figure has been chosen so as to facilitate the visualization of the disks (which can be fairly small in some cases) and, as such, is different in each plot. A typical Polish doughnut is represented by the model in the top-left panel. As the black hole spin increases, the maximum of the density (and of the pressure), which yields the center of the disk, moves inwards for all values of $\gamma$ and $\beta$. For $\gamma=0$ the disks are infinite, irrespective of the value of $a$. As $\gamma$ and $\beta$ increase from 0 to 0.9 (compare the three top rows) the disks become significantly thinner and radially smaller (see also Fig.~\ref{more-profile} below). \tf{Check if the description is correct. Add further relevant information.}

Figure~\ref{magnetization} shows the effects of changing the parameter $\beta_{\rm c}$ (the magnetization) in the structure of the disks. From left to right the values plotted in each panel are $\beta_{\rm c}=10^3$, 1, and $10^{-3}$. The model chosen corresponds to $a=0.9$ and $\gamma=\beta=0.5$. The larger the value of $\beta_{\rm c}$ the less important the effects of the magnetization in the disk structure. Figure~\ref{magnetization} shows that for the kind of magnetic field distribution we are considering, the effects of magnetization are minor. The disk structure remains fairly similar for all values of $\beta_{\rm c}$ and the only quantitative differences are found in the location of the centre of the disk (which moves inward with decreasing $\beta_{\rm c}$) and of the range of variation of the isodensity contours (the maximum being slightly larger with decreasing $\beta_{\rm c}$). This can be more clearly visualized in Figure~\ref{magnetization-profile} which displays the radial profile at the equatorial plane of the logarithm of the density \tf{Is it $\log\rho$ or $\rho$?} for the same three cases plotted in Fig.~\ref{magnetization}. \tf{Check if the description is correct. Add further relevant information.}

Next, we show in Fig.~\ref{more-profile} the corresponding radial profiles at the equatorial plane for the models with black hole spin $a=0.9$. We consider the case $\beta_{\rm c}=10^{-3}$ (highest magnetization) and the same combination of the $\gamma$ and $\beta$ parameters that we employed in Fig.~\ref{models}. Therefore, this plot depicts the radial profiles of the models occupying the central column of Fig.~\ref{models}. These profiles allow for a clearer quantification of the radial extent of the disks with $\gamma$ and $\beta$. As mentioned in the description of Fig.~\ref{models} as $\gamma$ and $\beta$ increase from 0 to 0.9 (compare black, blue, and green curves) the disks become gradually smaller. At the same time, the radial location of the centre increases and the maximum value of the central density (and pressure) decreases. It is worth noticing that the radial profiles of some of the models overlap below a certain radius. More precisely, the black and red curves overlap below $r\sim 4$ and also the green and orange curves. \tf{Check if the description is correct. Add further relevant information.}
 



 
In order to provide a quantitative comparison of the structural differences that appear along the sequence of equilibrium models, we plot in Fig.~\ref{max-vs-magnetization} the variation of the maximum of the density (left panel), the radial location of the maximum of the thermal pressure (middle panel), and the radial location of the maximum of the magnetic pressure (right panel) as a function of the magnetization parameter $\beta_{\rm c}$. All quantities plotted are measured at the equatorial plane. Let us first consider the left panel. The line type used indicates the value of the black hole spin $a$, namely, a dotted line is for $a=0.5$, a dashed line for $a=0.9$, and a solid line for $a=0.99$. Correspondingly, the colour indicates the value of $\beta$ and $\gamma$ used, namely, red curves correspond to $\gamma=\beta=0$, the blue ones to $\gamma=\beta=0.5$, and the black ones to $\gamma=\beta=0.9$. \tf{Is my interpretation of the line type and colour code correct?} All curves show the same monotonically decreasing trend with $\beta_{\rm c}$: for small enough and large enough values of $\beta_{\rm c}$ the value of the density maximum does not change. However, in the interval $10^{-2}\le \beta_{\rm c} \le 10^{2}$, $\rho_{\rm max}$ changes abruptly. The larger the black hole spin the larger the drop in the maximum of the density. For sufficiently large values of $\beta_{\rm c}$ (small magnetization) the value of $\rho_{\rm max}$ stays constant to the same value irrespective of the black hole rotation.

In the middle panel of Fig.~\ref{max-vs-magnetization} we show the radial location of the maximum of the thermal pressure as a function of $\beta_{\rm c}$. The black hole spin is $a=0.99$ \tf{is that correct?} and the values of $\beta$ and $\gamma$ are indicated in the legend. For all values of $\beta$ and $\gamma$, the maximum of the disk thermal pressure decreases with decreasing $\beta_{\rm c}$. Below $\beta_{\rm c}\sim 10^{-3}$ the location of the maximum does not change. The panel also shows that above $\beta_{\rm c}\sim 10^{3}$ the constant value of $r_{\rm max}$ is the same irrespective of $\beta$ and $\gamma$.

The dependence of the location of the maximum of the magnetic pressure with $\beta_{\rm c}$ is shown in the right panel of Fig.~\ref{max-vs-magnetization}. While the maximum of the magnetic pressure also decreases with decreasing $\beta_{\rm c}$, the constant value achieved for values above $\beta_{\rm c}\sim 10^{3}$ does depend on the specific values of $\beta$ and $\gamma$. It must be noted that, for the magnetic pressure, all models seem to cross at roughly the same value of $r_{\rm max}$, as it is more clearly shown in the inset.
...
We have discussed the properties of the new models and their dependence on the initial parameters. \tf{Expound on this presenting a short summary of our main findings.} 

\tf{To close the paper show a short outline of potential future work we plan to attempt with this research: inclusion of self-gravity as in the work of Stergioulas, and construction of disks around hairy black holes.}
