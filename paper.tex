...
\ln |u_t| - \int^l_0 \frac{\Omega \mathrm{d}l}{1 - \Omega l} + \int^p_0 \frac{\mathrm{d}p}{w} + \int^{\tilde{p}_{\mathrm{m}}} \frac{\mathrm{d}\tilde{p}_{\mathrm{m}}}{\tilde{w}} = \mathrm{const}.
\end{equation}
...
are satisfied and, therefore, the integration constant is given by
\begin{equation}
...
\end{equation}
where $W_{\mathrm{in}}$ is the potential on the inner edge of the disk (in the equatorial plane). With this definition, we can write \eqref{eq:pre_full_int} as
