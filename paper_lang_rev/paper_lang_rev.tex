...
The organization of the paper is as follows: \sg{Sect.}~\ref{framework} presents the analytic framework to build the discs while \sg{Sect.}~\ref{methodology} explains the corresponding numerical procedure. The sequences of models are discussed in \sg{Sect.}~\ref{results}. Finally, the conclusions are summarized in \sg{Sect.}~\ref{conclusions}, where we also briefly indicate potential extensions of this work we plan to inspect. In our work we assume that the test-fluid approximation holds, thus neglecting the self-gravity of the fluid, and further assume that the spacetime is described by the Kerr metric. In mathematical expressions below Greek indices are spacetime indices running from 0 to 3 and Latin indices are only spatial. We use geometrized units where $G = c = 1$.
...
$ and the only quantitative differences are found in the location of the centre of the disc (which moves inward with decreasing $\beta_{\mathrm{m}_{\mathrm{c}}}$) and of the range of variation of the isodensity contours (the maximum being slightly larger with decreasing $\beta_{\mathrm{m}_{\mathrm{c}}}$). This can be more clearly visualized in \sg{Fig.}~\ref{magnetisation-profile} which displays the radial profile at the equatorial plane of the rest-mass density for the same three cases plotted in Fig.~\ref{magnetisation}.
