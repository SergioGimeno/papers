...
{\bf A:} We appreciate the comment of the referee and apologize if the discussion is Section 4 is not sufficiently clear. We have been
analyzing in more detail our findings and have come up with the following simple explanations for our numerical observations.

\bigskip

To explain the movement of the maximum of the rest-mass density towards smaller $r$ we can start by computing the radial derivative of the enthaply, which should vanish at $r=r_{\rm max}$. From Eq.~(20) we have
\begin{eqnarray*}
\partial_r w=\partial_r\left[
-\Delta W \left( \frac{\lambda-1}{\lambda}\right)\frac{1}{K+K_m \mathcal{L}^{\lambda-1}}
\right]^{\frac{1}{\lambda-1}}\,,
\end{eqnarray*}
where we have used $\kappa=\lambda$ and have defined $\Delta W\equiv W-W_{\rm in}$. After some algebra, we arrive at the following expression for the radial derivative of the potential:
\begin{eqnarray*}
\partial_r W(r)|_{r=r_{\rm max}}=\frac{(\lambda-1) \Delta W(r_{\rm max})}{\mathcal{L}(r_{\rm max})}
\left[
\frac{(\partial_r \mathcal{L})|_{r=r_{\rm max}}}{1+\beta_{{\rm m}_{\rm c}}\left(\frac{\mathcal{L}(r_c)}{\mathcal{L}(r_{\rm max})} \right)^{\lambda-1}}
\right]\,.
\end{eqnarray*}
All the factors on the r.h.s.~of this expression are positive, except for $\Delta W(r_{\rm max})$ which is negative. For $\beta_{{\rm m}_{\rm c}}\rightarrow \infty$,  $\partial_r W(r)|_{r=r_{\rm max}}=0$, which is attained at $r_{\rm max}=r_c$, which is a minimum of the potential.
As $\beta_{{\rm m}_{\rm c}}$ decreases (and hence the magnetisation increases) the r.h.s.~increases in absolute value being always negative. Therefore, the position of $r_{\rm max}$ has to move towards smaller radii.
