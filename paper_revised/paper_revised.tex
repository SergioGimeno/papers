...
\end{equation}
With both pressures known at the centre, fluid and magnetic, we can now find the constants $K$ and $K_{\mathrm{m}}$ using equations \eqref{eq:eos_fluid} and \eqref{eq:eos_mag}. Therefore, for a given inner radius of the disc $r_{\mathrm{in}}$ we can obtain the potential $W_{\mathrm{in}}$. We now finally have all the ingredients required to find the enthalpy distribution (Eq.~\eqref{eq:enthalpy_eq}), the fluid pressure and magnetic pressure distributions (Eqs.~\eqref{eq:eos_fluid} and \eqref{eq:eos_mag}), and the {\bf rest-mass} density distribution, which can be trivially obtained inverting the barotropic relation, $p = K \rho^{\kappa}$. {\bf We note that the specific internal energy $\varepsilon$ is needed to compute the enthalpy. We obtain
...
Eq.~\eqref{eq:polar_der_pot} of the paper gives us
...
To evaluate this expressión, we need to compute the partial derivatives $\partial_r \mathcal{L}$ and $\partial_r w$. The derivative of $\mathcal{L}$ is esay to compute, as $\mathcal{L} = \Delta \sin^2 \theta$. So its partial derivative is

\begin{equation}
\partial_r \mathcal{L} = 2(r-M)\sin^2 \theta
\end{equation}

Now, we take our attention to $\partial_r w$. From eq.~\eqref{eq:enthalpy_eq} we have
...
\begin{equation}
\partial_r w = \frac{w}{\lambda - 1} \left[-\Delta W \left(\frac{\lambda -1}{\lambda}\right)\frac{1}{K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}}\right]^{-1} \left[\left(-\partial_r W \left(\frac{\lambda -1 }{\lambda}\right) \frac{1}{K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}}\right) + \left(-\Delta W \left(\frac{\lambda -1}{\lambda}\right)\frac{-(\lambda - 1) K_{\mathrm{m}} \mathcal{L}^{\lambda - 2} \partial_r \mathcal{L}}{(K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1})^2}\right)\right]
\end{equation}

Since we are at $r = r_{\mathrm{c}}$ the derivative of the potential is zero ($\partial_r W(r, \pi/2)|_{r = r_{\mathrm{c}}} = 0$), so we can simplify the expression to get

\begin{equation}
\partial_r w = \frac{w}{\lambda - 1} \left[-\Delta W \left(\frac{\lambda -1}{\lambda}\right)\frac{1}{K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}}\right]^{-1} \left(-\Delta W \left(\frac{\lambda -1}{\lambda}\right)\frac{1}{K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}}\right)\left[\frac{-(\lambda - 1) K_{\mathrm{m}} \mathcal{L}^{\lambda - 2} \partial_r \mathcal{L}}{K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}}\right]
\end{equation}

And we can further simplify this, to arrive at

\begin{equation}
\partial_r w = - w \left[\frac{K_{\mathrm{m}} \mathcal{L}^{\lambda - 2} \partial_r \mathcal{L}}{K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}}\right]
\end{equation}

Then, we can insert this expression at eq.~\eqref{eq:simple_expr} to obtain

\begin{equation}
(\partial_r \mathcal{L}) w [(\lambda -1)(K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}) - \lambda K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}] = 0
\end{equation}

As $(\partial_r \mathcal{L}) w \neq 0$ inside the disc, we can simplify this, to arrive at 

\begin{equation}
K (\lambda - 1) = K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}
\end{equation}

The, we can use equations \eqref{eq:eos_fluid}, \eqref{eq:eos_mag} and \eqref{eq:beta_eq} at the center (remember that $w_{\mathrm{c}} = 1$) to write $K$ as $K = \beta_{\mathrm{m}_{\mathrm{c}}} K_\mathrm{m}\mathcal{L}^{\lambda -1}$. This allows us to write the simple expression

\begin{equation}
\beta_{\mathrm{m}_{\mathrm{c}}} = \frac{1}{\lambda - 1}
\end{equation}

\sg{We should add a short discussion of the generality of this result.}
