...
%{\bf We note that the specific internal energy $\varepsilon$ is needed to compute the enthalpy. We obtain
...
\nonumber \\
&\times&
\left[\left(-\partial_r W \left(\frac{\lambda -1 }{\lambda}\right) \frac{1}{K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}}\right) + \left(-\Delta W \left(\frac{\lambda -1}{\lambda}\right)\frac{-(\lambda - 1) K_{\mathrm{m}} \mathcal{L}^{\lambda - 2} \partial_r \mathcal{L}}{(K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1})^2}\right)\right]\,.
\end{eqnarray}
...
\nonumber \\
&\times&
\left(-\Delta W \left(\frac{\lambda -1}{\lambda}\right)\frac{1}{K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}}\right)\left[\frac{-(\lambda - 1) K_{\mathrm{m}} \mathcal{L}^{\lambda - 2} \partial_r \mathcal{L}}{K+K_{\mathrm{m}}\mathcal{L}^{\lambda - 1}}\right]\,,
\end{eqnarray}
...
\end{equation}
Moreover, we can use Eqs.~\eqref{eq:eos_fluid}, \eqref{eq:eos_mag} and \eqref{eq:beta_eq} at the center (remember that $w_{\mathrm{c}} = 1$) to write $K$ as $K = \beta_{\mathrm{m}_{\mathrm{c}}} K_\mathrm{m}\mathcal{L}^{\lambda -1}$. Therefore, this allows us to obtain the following simple expression for the value of the magnetisation parameter at the center of the disc
\begin{equation}
...
\sg{It is relevant to note that the only reference to the explicit form of the metric it is done to show that $\mathcal{L} \neq 0$ and $\partial r \mathcal{L} \neq 0$ inside the disk. This means that our result holds for any stationary and axisymmetric metric if the aforementioned inequalities hold.

Also, if the angular momentum distribution allows the existence of a cusp and a center (and $r_{\mathrm{c}} \neq r_{\mathrm{cusp}}$) the result also holds for any angular momentum distribution.}
